...
class LlmChatLogGeneratorLocal < StringEncoder
  ...
  def initialize
    super
    self.module_name = 'LLM Chat Log Generator'
    self.organisation = '{}'
    self.theme = 'insider_threat'
    self.chat_type = 'team_channel'
    self.seed = nil
    self.llm_model = ''
    self.participants = []
  end
  ...
  def encode_all
    ...
    generator = LlmChatLogNarrativeGenerator.new(options)
  ...
  def get_options_array
    super + [['--organisation', GetoptLong::REQUIRED_ARGUMENT],
             ...
             ['--chat_type', GetoptLong::OPTIONAL_ARGUMENT],
  ...
  def build_participants(org_data)
    ...
    if org_data['employees'].is_a?(Array)
      org_data['employees'].each { |emp| result << emp.merge('role' => 'Employee') }
    end
    result
  end
end

class LlmChatLogNarrativeGenerator < LlmNarrativeGenerator
  def build_prompt(extra_variables = {})
    vars = template_variables(extra_variables)
    prompt = LlmPromptTemplate.load_and_render('chat_log', vars)
    append_cybok_alignment(prompt)
  end
end

LlmChatLogGeneratorLocal.new.run
