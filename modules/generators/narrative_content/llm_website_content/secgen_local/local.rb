...
class LlmWebsiteContentGeneratorLocal < StringEncoder
  ...
  def initialize
    super
    self.module_name = 'LLM Website Content Generator'
    self.organisation = '{}'
    self.theme = 'investigation'
    self.content_type = 'company_info'
    self.seed = nil
    self.llm_model = ''
  end
  ...
  def encode_all
    ...
    generator = LlmWebsiteContentNarrativeGenerator.new(options)
  ...
  def get_options_array
    super + [['--organisation', GetoptLong::REQUIRED_ARGUMENT],
             ...
             ['--content_type', GetoptLong::OPTIONAL_ARGUMENT],
...
class LlmWebsiteContentNarrativeGenerator < LlmNarrativeGenerator
  def build_prompt(extra_variables = {})
    vars = template_variables(extra_variables)
    prompt = LlmPromptTemplate.load_and_render('website_content', vars)
    append_cybok_alignment(prompt)
  end
end

LlmWebsiteContentGeneratorLocal.new.run
