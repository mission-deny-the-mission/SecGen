#!/usr/bin/ruby
require 'json'
require_relative '../../../../../lib/objects/local_string_encoder.rb'
require_relative '../../lib/llm_narrative_generator.rb'

class LlmDatabaseRecordGeneratorLocal < StringEncoder
  attr_accessor :organisation
  attr_accessor :theme
  attr_accessor :record_type
  attr_accessor :num_records
  attr_accessor :seed
  attr_accessor :llm_model

  def initialize
    super
    self.module_name = 'LLM Database Record Generator'
    self.organisation = '{}'
    self.theme = 'insider_threat'
    self.record_type = 'customer_data'
    self.num_records = '20'
    self.seed = nil
    self.llm_model = ''
  end

  def encode_all
    org_data = JSON.parse(self.organisation) rescue {}

    options = {
      'seed' => self.seed,
      'model' => self.llm_model.empty? ? nil : self.llm_model,
      'theme' => self.theme,
      'organisation' => org_data,
      'content_type' => 'database_record'
    }

    generator = LlmDatabaseRecordNarrativeGenerator.new(options)
    content = generator.generate_content({
      'record_type' => self.record_type,
      'num_records' => self.num_records
    })

    self.outputs << content
  end

  def get_options_array
    super + [['--organisation', GetoptLong::REQUIRED_ARGUMENT],
             ['--theme', GetoptLong::REQUIRED_ARGUMENT],
             ['--record_type', GetoptLong::OPTIONAL_ARGUMENT],
             ['--num_records', GetoptLong::OPTIONAL_ARGUMENT],
             ['--seed', GetoptLong::OPTIONAL_ARGUMENT],
             ['--llm_model', GetoptLong::OPTIONAL_ARGUMENT]]
  end

  def process_options(opt, arg)
    super
    case opt
    when '--organisation'
      self.organisation = arg
    when '--theme'
      self.theme = arg
    when '--record_type'
      self.record_type = arg
    when '--num_records'
      self.num_records = arg
    when '--seed'
      self.seed = arg.to_i
    when '--llm_model'
      self.llm_model = arg
    end
  end
end

class LlmDatabaseRecordNarrativeGenerator < LlmNarrativeGenerator
  def build_prompt(extra_variables = {})
    vars = template_variables(extra_variables)
    prompt = LlmPromptTemplate.load_and_render('database_record', vars)
    append_cybok_alignment(prompt)
  end
end

LlmDatabaseRecordGeneratorLocal.new.run
