...
class apache::params inherits ::apache::version {
  ...
  if ($apache::version::scl_httpd_version) {
    ...
    if ($facts["os"]["name"] == 'Ubuntu' and versioncmp($facts["os"]["release"]["full"], '19.04') < 0) or ($facts["os"]["name"] == 'Debian' and versioncmp($facts["os"]["release"]["full"], '10') < 0) {
    ...
    $secpcrematchlimitrecursion = 1500
    $modsec_secruleengine = 'On'
    if ($facts["os"]["name"] == 'Debian' and versioncmp($facts["os"]["release"]["full"], '9') >= 0) or ($facts["os"]["name"] == 'Ubuntu' and versioncmp($facts["os"]["release"]["full"], '18.04') >= 0) {
      $modsec_default_rules = [
    ...
    $mpm_module          = 'prefork'
    if versioncmp($facts["os"]["release"]["full"], '15') < 0 {
    ...
    $suphp_addhandler    = 'x-httpd-php'
    $suphp_engine        = 'off'
    if versioncmp($facts["os"]["release"]["full"], '11') < 0 or versioncmp($facts["os"]["release"]["full"], '12') >= 0 {
      $mod_packages = {
